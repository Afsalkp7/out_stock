    ...
    btn.addEventListener('click', async (event) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          <input type="text" class="form-control" id="name" value="${brandData.brandName}" name="brandName">
          </div>
        <div class="form-floating mb-3 mt-3">
          <input type="text" class="form-control" id="floatingPassword" value="${brandData.description}" name="description">
        </div>
        <div class="form-floating mb-3 mt-3">
      ...
      <button type="button" onclick="updateBrand()" data-bs-toggle="modal" data-bs-dismiss="modal" class="btn btn-dark text-light mt-3">Submit</button>
          ...
          // Show the modal
          const catModal = new bootstrap.Modal(document.getElementById('brandShow'));
          catModal.show();
        } else {
          console.error('Error fetching user data');
        }
